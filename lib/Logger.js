...
class Logger {
    ...
    _log(type, ...args) {
        ...
        for ( const key of this.#formatter.keys ) {
            switch (key) {
            case "TYPE":
                params[key] = this.#f(type.length < 5 ? type+" " : type,{ color:this.type_to_color(type), bold:true });
                break;
            case "NAMESPACE":
                params[key] = this.#f(this.#namespace,{ color:"#4bbacb" }); // light cyan
                break;
            case "STRING":
                // Nothing to do here
                break;
            case "ISO":
                ...
                break;
            case "LTIME":
                params[key] = this.#f((new Date()).toLocaleTimeString('es-US', LTIME_OPTS),{ color:"#7CFC00" }); // grass green
                break;
            default:
                params[key] = "???";
            }
    ...
    /**
     * Create a new logger that appends to a file
     *
     * @static
     ...
     * @returns {Logger} The new logger
     */
    static append_file(file_path, namespace="", opts={}) {
        return new this({
            log : {
                debug : (string) => appendFileSync(file_path, string + "\n", opts),
                info : (string) => appendFileSync(file_path, string + "\n", opts),
                warn : (string) => appendFileSync(file_path, string + "\n", opts),
                error : (string) => appendFileSync(file_path, string + "\n", opts),
            },
            namespace,
            ...opts
        });
